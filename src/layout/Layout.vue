          ...
          >
            <div style="display: flex">
              ...
              <ElDropdown style="margin-left: 10px" trigger="click">
                ...
                <template #dropdown>
                  <ElDropdownMenu>
                    <ElDropdownItem @click="createNewHistory"
                      >新建</ElDropdownItem
                    >
                    <ElDropdownItem @click="saveAsNewHistory"
                      >另存</ElDropdownItem
                    >
              ...
              <ElDropdown style="margin-left: 10px" trigger="click">
                ...
                <template #dropdown>
                  <ElDropdownMenu>
                    <ElDropdownItem @click="createNewScene"
                      >新建</ElDropdownItem
                    >
                    <ElDropdownItem @click="saveAsNewScene"
                      >另存</ElDropdownItem
                    >
          ...
          <ElRow>
            ...
            <ElCol :span="4">
              ...
              >
                <template #trigger>
                  <ElButton
                    style="height: 100%; width: 120px; margin-left: 10px"
                    :disabled="running"
                    >上传图片
                  </ElButton>
                </template>
...
const upload = (file: any) => {
  ...
  reader.onload = () => inputImage.value = reader.result as string;
  ...
  if (inputMessage.value) {
    ...
    baklava.value
      ?.runOnce({
        ...
        exec: true,
        history: chat.value?.getHistory(),
        image_url: inputImage.value,
      })
      .then((res: any) => {
        if (res) {
          chat.value?.appendMessage({
            role: "user",
...
const exportData = () => {
  exportJson(
    { graphs: graphStore.graphs, histories: historyStore.histories },
    "data.json"
  );
};
/**
  ...
  if (graphStore.currentGraph == -1 || historyStore.currentHistory == -1) {
    dialog.value?.openDialog("保存", (input: string) => {
      if (graphStore.currentGraph == -1) {
        ...
        if (baklava.value) {
          graphStore.getGraph(graphStore.currentGraph).state =
            baklava.value.getGraph();
        }
      }
      graphStore.save();
      if (historyStore.currentHistory == -1) {
        ...
        if (chat.value) {
          historyStore.getHistory(historyStore.currentHistory).history =
            chat.value.getHistory();
        }
      }
      historyStore.save();
    });
